...
### Phase 2 — Tools subsystem redesign (10 commits, 2715→2743 tests, agent.py 1841→1697)

Typed `ToolMetadata` replaces the untyped metadata dict; `StatefulToolFactory` registry replaces the 64-line if/elif in `Agent.__init__`; `tools.mcp_overrides_builtin` config flag opens up MCP-vs-builtin collision policy; `McpRuntime` extracted to `aura/core/runtime/mcp.py` (-98 LoC from `agent.py`); tool errors normalized to one `ToolResult` shape end-to-end (loop, transport, frontend).

- **`feat(schemas): define ToolMetadata dataclass for tool capability flags`** (`91d448d`): 8-field frozen dataclass replacing `dict[str, Any]`. `tool_metadata(...)` helper continues to return the legacy dict during migration.
- **`refactor(tools): pilot ToolMetadata migration on read_file`** (`d437bd5`): pilot tool sets `aura_metadata: ToolMetadata`; loop / hook / permission readers unified through `meta_dict(tool)` bridge in `aura/schemas/tool_meta_access.py` that prefers typed slot then falls back to legacy dict.
- **`refactor(tools): migrate remaining 20 builtins to ToolMetadata`** (`2eca905`): every builtin migrated. `ToolMetadata.is_destructive` widened to accept `bool | ToolFlagResolver` callable for tools (e.g. bash) using dynamic destructiveness classification.
- **`refactor(registry): enforce ToolMetadata at registration`** (`e206c7f`): `ToolRegistry.register` raises `ToolRegistryError` (subclass of `AuraError`) at registration when `aura_metadata` is missing or wrong type. `meta_dict()` legacy `tool.metadata` fallback deleted; reads only typed slot. `mcp/adapter.py` writes `aura_metadata` not `tool.metadata`. `ToolMetadata.max_result_size_chars` promoted into the typed surface.
- **`feat(tools): introduce StatefulToolFactory Protocol with TodoWriteFactory pilot`** (`6eb8fe8`): `aura/core/runtime/tool_factory.py` defines a `runtime_checkable` Protocol + `ToolRuntime` frozen dataclass exposing the dependencies stateful tools need (state, asker, tasks_store, subagent_factory, running_tasks, running_shells, transcript_storage, agent, team_manager). `TodoWriteFactory` pilot proves the pattern.
- **`refactor(agent): replace stateful-tool if/elif with factory registry`** (`753d9ae`): 6 more factories (AskUser / TaskCreate / TaskGet / TaskList / TaskStop / SendMessage) plus a `STATEFUL_TOOL_FACTORIES` constant. `Agent.__init__` collapses ~110 lines of if/elif into one for-loop. agent.py shrinks 1841 → 1790 (-51 LoC). Six tools (`task_output`, `web_search`, `enter_plan_mode`, `exit_plan_mode`, `bash_background`, `skill`) stay wired inline because they need closures over `self`-bound methods that don't fit a frozen `ToolRuntime`.
- **`feat(tools): mcp_overrides_builtin config flag for collision policy`** (`9ca7a63`): `tools.mcp_overrides_builtin: bool = False` (default preserves builtin-wins). `assemble_tool_pool(builtins, mcp_tools, *, mcp_overrides=...)` honors the flag; `mcp_tool_shadowed` event emits `winner="builtin"` or `winner="mcp"` for both directions of the collision.
- **`refactor(agent): extract McpRuntime from Agent`** (`ed88e21`): `aura/core/runtime/mcp.py` (314 LoC) owns the `MCPManager` reference, mcp_commands list, tools snapshot, `connect_all` (with graceful failure journaling — `mcp_aconnect_failed` / `mcp_aconnect_done`), `disconnect_all` (B3 timeout-bounded teardown — `mcp_close_timeout` / `mcp_close_error` / `mcp_stopped`), `connected_server_names`, and `replace_registry_contents`. agent.py shrinks 1795 → 1697 (-98 LoC). Back-compat property shims for `_mcp_manager` / `_mcp_commands` keep the ~5 test files that poke private attrs working without test edits.
- **`refactor(loop): normalize tool errors to single ToolResult shape`** (`799b04f`): one tool-result code path. The dual `try/except ToolError` / `except Exception` branches collapse to a single defensive `except Exception → ToolResult(error=True)` per spec §7. AG-UI `TOOL_CALL_RESULT.content` flips to structured `{"text": str, "error": bool}` (legacy `output: any, error: str | null` split fields dropped). Frontend `ToolCard.tsx` renders error path with `tool-card--error` modifier (terracotta top border + ⊘ glyph) per Phase 9 mockup §4.5. Bundle size delta: +14 bytes gzip.

### Phase 3 — Context & Memory redesign (6 commits, 2743→2770 tests)

`Context.fresh()` factory replaces implicit "construct new instance" reset; typed `ReadCarryover` + `ReadRecord` carry inherited file-read freshness across the parent/subagent boundary; cwd-boundary check unified between nested-memory load and rule-match.

- **`feat(schemas): ReadCarryover with mtime-based freshness check`** (`48459e0`): `ReadRecord` (path / mtime_at_read / size_at_read / read_at_turn) and `ReadCarryover` (records / source_session_id / generated_at_turn) frozen dataclasses. `ReadCarryover.is_fresh(path)` re-stats and compares to recorded values. `MappingProxyType` enforces immutable-view on `records`.
- **`feat(context): explicit fresh() factory replaces implicit Context(...) reset`** (`f333d5d`): `Context.fresh(*, carryover=None, clear_reads=False)` returns a new instance with cleared progressive state (loaded_nested_paths, nested_fragments, matched_rule_paths, matched_rules, invoked_skills) and preserved config. Defaults preserve `_read_records` (file is still on disk after compact).
- **`refactor(compact): use Context.fresh() for post-summary reset`** (`8523390`): drops the `Context(...)` constructor call from `compact.py`; uses `old_ctx.fresh()`. Side-fix: previous reconstruction silently dropped `notifications_drainer`; `fresh()` preserves it.
- **`refactor(tasks): typed ReadCarryover replaces inherited_reads dict`** (`5e0c37b`): `Context.__init__` accepts `carryover: ReadCarryover | None` (no more `inherited_reads: dict`). `Agent._snapshot_read_carryover()` builds the carryover at subagent spawn time. `SessionRuntime.carryover` property replaces `inherited_reads`. `SubagentFactory.parent_carryover_provider` callable replaces `parent_read_records_provider`.
- **`fix(perm): must_read_first checks carryover freshness for subagents`** (`af9c861`): when a subagent inherits a parent's read record but the file's mtime/size has changed since, `Context.read_status` returns `"stale"` and `must_read_first` blocks with "has changed since last read." Three new tests pin: live-read fresh path / inherited-read fresh path / inherited-read stale path / inherited-read missing-file path.
- **`fix(memory): unify cwd-boundary for nested-memory and rule-match`** (`d25e23b`): single `_path_in_scope(path)` method gates BOTH nested-memory load and rule-match. Out-of-cwd paths now skipped consistently (previously rule-match ignored cwd boundary). `out_of_cwd_rule_warning` journal event fires once at rule load for any rule with absolute glob patterns outside the project root, surfacing the common "rule won't match anything" silent-failure.

### Phase 4 — Compact subsystem redesign (7 commits, 2770→2801 tests)

`CompactionTrigger` enum tags every compaction; `CompactConfig` lifts hardcoded constants to user-configurable surface; concrete `Compactor` class replaces the Phase 1 `LegacyCompactor` adapter; AG-UI `aura.compact.event` lifecycle events emit per trigger so frontends see compaction in real time.

- **`feat(compact): CompactionTrigger enum + CompactConfig schema`** (`4904f6b`): `CompactionTrigger(StrEnum)` with `microcompact / reactive / auto / manual`. `CompactConfig` (11 fields, all `extra="forbid"`) replaces hardcoded constants for file caps, summary char limits, recursion depth, circuit-breaker threshold, microcompact pair counts, and the optional time-based gap trigger.
- **`refactor(compact): migrate file caps to CompactConfig`** (`7d7f1b4`): six hardcoded constants in `compact.py` / `microcompact.py` move to `agent.config.compact.<field>`. Override-via-config integration test confirms `max_files_to_restore=2` clamps a 5-file restore to 2.
- **`feat(compact): Compactor class with trigger-tagged methods`** (`e687fb4`): real `Compactor` class at `aura/core/compact/compactor.py`. Four async methods (`microcompact / reactive / auto / manual`) each emit one `compact_event` journal record + one wire-format dict to an optional `event_emitter` callable. Circuit breaker reads `config.max_consecutive_failures`, writes `slots.consecutive_compact_failures` via `dataclasses.replace`. Manual bypasses the breaker. New `aura/transport/wire.py:compact_event_to_wire` helper + AG-UI mapping to `{"type": "CUSTOM", "name": "aura.compact.event", "value": {trigger, tokens_before, tokens_after, outcome, duration_ms}}`.
- **`refactor(loop): replace LegacyCompactor with Compactor`** (`8d65449`): `Agent._build_loop` instantiates the real `Compactor` (not LegacyCompactor) and wires the `event_emitter` to a per-Agent `_pending_compact_events` buffer that `astream` drains alongside the typed `AgentEvent` stream. `Agent.astream` widened to `AsyncIterator[AgentEvent | dict[str, Any]]` so compact lifecycle events flow through one channel; CLI renderer skips `dict` events (compact lifecycle is silent in CLI; visible via AG-UI in desktop). `transport.wire.event_to_wire` passes already-wire-format dicts through verbatim.
- **`refactor(compact): delete LegacyCompactor adapter`** (`a28a170`): the Phase 1 transitional shim is gone. The Phase 1 `Compactor` Protocol conformance test repoints to the real class. Net: -201 LoC.
- **`test(compact): pin precedence rules across the three triggers`** (`0ebc69a`): 5 new tests lock the interaction semantics — microcompact always runs first; reactive only on `PromptTooLong`; auto only post-turn; length-recovery and reactive disjoint; circuit breaker after 3 consecutive failures resets on success. Implementation note: the breaker silently skips with `auto_compact_skipped_circuit_breaker` journal event (does NOT raise per spec sketch — implemented behavior is what's pinned).



### Audit-MED-clearance (Round 9)

40 OPEN MED findings cleared, 0 deferrals. Verified by 5 independent strict re-audit subagents.

- **F-01-004 / 005 / 013** (`fix(retry): F-01-004/005/013`): retry honours HTTP `Retry-After` header (with 5-min cap), partial-response recovery on `finish_reason=length` / `stop_reason=max_tokens` (3-attempt resume), `BaseException` narrowed to `Exception` so KeyboardInterrupt / SystemExit propagate.
- **F-01-012** (`fix(loop,agent)`): reactive recompact moved INSIDE `AgentLoop._invoke_model` via `compact_callback` — turn_count + per-turn audit state preserved across the recompact retry.
- **F-02-003 / 005 / 009** (`fix(tools): read_file/edit_file`): `read_file` head-truncates instead of rejecting at the byte cap (`partial=True` + `truncated_at_bytes`); UTF-16 LE/BE + UTF-8 BOM detection; `edit_file` rejects targets > 256 MB pre-stat.
- **F-02-016 / 035 + F-04-008** (`fix(bash)`): regex coverage extended to `$(...)` / backtick command substitutions wrapping `rm` / `chown -R` / `dd` / `mkfs` / `chmod 777`, `find -exec` invoking destructive verbs, bare `dd of=/dev/*` and `mkfs /dev/*`; `start_new_session=True` (POSIX) + `CREATE_NEW_PROCESS_GROUP` (Windows) so Ctrl-C signals the child's process group; `bash_safety._is_system_path` resolves brace expansion (`/{etc,tmp}` → cartesian product, nested supported).
- **F-02-019 / 020 / 021** (`fix(tools): grep/glob`): grep auto-excludes `.git` / `.svn` / `.hg`, caps lines at `--max-columns=500` (override-able); glob honours `.gitignore` via `git ls-files` intersection inside repos, falls back to `Path.glob` outside.
- **F-02-031 + F-04-007** (`fix(registry,rule)`): `assemble_tool_pool(builtins, mcp_tools)` resolves built-in vs MCP collisions with builtin-precedence + `mcp_tool_shadowed` journal events; `Rule.parse` tool names validated at config load against the known-tool set with `difflib.get_close_matches` hint (CLI passes `config.tools.enabled + ["mcp__*"]`).
- **F-03-003 / 005 / 006 / 007 / 008** (`fix(memory)`): project-memory wrapped in `<system-reminder>` envelope with claude-code's exact OVERRIDE preamble; `paths: ["**"]` rules normalize to unconditional; walk-up caps at git root (subprocess `git rev-parse --show-toplevel`); truncation marker is now an explicit `WARNING: this file is N bytes (limit: 25000) ...`; `@import` paths gated by extension allowlist (`.md` / `.txt` / `.py` / `.json` / `.yaml` / `.toml` / `.sh` / `.cfg` / `.ini`).
- **F-03-004** (`feat(memory)`): per-project auto-memory directory at `<projects>/<encoded-cwd>/memory/`. `Storage.memory_dir(cwd=...)` resolves the path; `load_project_memory(auto_memory_dir=...)` includes its `MEMORY.md` index in the eager chain; the system prompt includes a `# auto memory` section documenting the convention (4 types: user / feedback / project / reference; what NOT to save; how to save with frontmatter).
- **F-04-002** (`fix(perm)`): `PreToolHook` can now return `ask` to demote a previously-allowed call back to the asker. Merge precedence is `deny > ask > allow`. `state.custom["_pre_tool_ask_pending"]` carries the signal across the chain.
- **F-04-003** (`fix(perm)`): documented deferral — Aura intentionally has no LLM-driven bash classifier (latency / cost trade-off; `bash_safety.py` already hard-blocks the genuinely-dangerous calls). See `aura/core/permissions/__init__.py` module docstring.
- **F-04-005** (`fix(perm)`): `_PLAN_MODE_READ_TOOLS` no longer admits `web_fetch` / `web_search` — plan mode means no side effects.
- **F-04-011** (`fix(hooks)`): `must_read_first` now also gates `bash` when the command contains a mutation idiom (`sed -i`, `> path`, `>> path`, `tee`, `tee -a`) targeting an existing file. Detection is shlex-based with separator-aware segmentation.
- **F-04-015 / 020** (`fix(perm)`): `disable_bypass=True` now clamps a runtime-requested `mode=bypass` back to `default` with a one-shot warning journal event (the closure consults the flag every call); `safety_exempt` patterns are validated at config load and reject any overlap with `DEFAULT_PROTECTED_READS / WRITES` via per-file `AuraConfigError`.
- **F-06-002 / 006 / 007** (`fix(mcp)`): `_build_one_connection` expands `${VAR}` / `${VAR:-default}` in `command` / `args` / `env` / `headers` / `url` (raises with the missing var name on no-default); MCP tool annotations (`readOnlyHint` / `destructiveHint` / `openWorldHint`) override conservative defaults when present; new test pins the `mcp__` namespace invariant against built-in collisions.
- **F-06-004** (`fix(mcp)`): every MCP connection carries `session_kwargs.message_handler` that journals `notifications/{tools,prompts,resources}/list_changed` per server. (Auto-rediscovery loop deferred — Aura's MCP sessions are short-lived; a persistent-session rearchitecture would unlock that. The journal trail is the observable surface in the meantime.)
- **F-07-004 / 005** (`fix(tasks)`): subagent recursion is no longer hard-banned — controlled at depth 2 (claude-code's contract). `SubagentFactory` carries a `_depth`; `spawn` raises `ToolError` at the cap; capped children have `task_create` / `task_output` stripped. Parent abort cascades via `parent_abort_event` + a watcher coroutine that cancels the child's `run_task` when the parent fires.
- **F-0910-001 / 002 / 003 / 004 / 008 / 011 / 016 / 020** (`fix(compact,skills,commands)`): model-aware auto-compact threshold (`auto_compact_threshold_for(model_spec) = window - 13_000`); 3-failure circuit breaker on auto-compact; PromptTooLong retry with 20%-drop strategy (3 attempts, hint to `/clear` on exhaustion); time-based microcompact trigger (`gap_threshold_minutes`); skills re-injected post-compact rebuild as `<skill-active>` HumanMessages capped at 5K tokens; bundled-skills layer with 3 real skills (`verify`, `simplify`, `code-review`); `/context` command surfaces per-section token estimates; non-terminal subagent tasks emit `<active-task>` markers across the rebuild boundary.

### Post-v0.18.0 patch fixes (7+ commits since the v0.18.0 tag)

These shipped after the tag was pushed but before the next release window. Functionally they are bug-fix follow-ups to issues operators hit in real sessions; will fold into v0.18.1 / v0.19.0.

- **`fix(perm)` (`354eb7b`)** — rule content supports `*` / `?` glob via `fnmatchcase`. Previously `bash(npm install fastapi)` only matched that exact command, so users had to author one rule per variation; one rule like `bash(npm install *)` now covers the whole family. Back-compat preserved when no metachar is present.
- **`fix(audit-strict)` (`aa28a12`)** — F-04-007 CLI wiring (`load_ruleset(known_tool_names=...)` now actually fires from CLI startup, surfacing typo'd tool names with `difflib.get_close_matches` hints), F-04-008 brace expansion (`rm -rf /{etc,tmp}` now resolves both alternatives), F-02-016 command-substitution coverage extended (`$(rm)` / `` `dd` `` / `$(chown -R)` / `$(mkfs)`), F-06-004 MCP `notifications/{tools,prompts,resources}/list_changed` handlers journal each event so operators can observe upstream catalog drift.
- **`fix(compact)` (`112e84b`)** — Aliyun DashScope error code 1261 ("Prompt exceeds max length") and the prose "prompt exceeds max length" / "input too long" / Gemini "request payload size exceeds" now trip `_is_context_overflow` so reactive recompact actually fires (used to silently propagate to the REPL). Structured-code fallback (`'code': '1261'`) catches translated/localized SDK errors. Estimator fallback (`_estimate_history_tokens` for providers without `usage_metadata`) now adds the pinned prefix size — a 5–10k underestimate previously let DashScope sessions overflow before auto-compact armed.
- **`fix(llm)` (`4cad5e2`)** — context-window table extended with real DeepSeek (`deepseek-chat` 128k / `deepseek-reasoner` 64k), Zhipu GLM (`glm-4` / `glm-4.5` / `glm-5` 128k, `glm-4-long` 1M), Aliyun Qwen (`qwen-plus` / `qwen-turbo` 128k, `qwen-turbo-1m` 1M, `qwen-long` 10M, `qwen-max` 32k), Moonshot Kimi (`kimi-k2` / `moonshot-v1-128k` 128k, `moonshot-v1-32k` 32k) family entries. Unknown-model default lowered 512k → 128k (claude-code parity) — the prior generous default lied to operators on common providers, showing 8% utilization right before the model rejected the prompt.
- **`feat(desktop): Phase 1` (`9781fc9`)** — Tauri 2 desktop app at `desktop/`; Rust IPC bridge spawns `python -m aura.cli.headless` as a subprocess and forwards NDJSON events to a vanilla TS frontend rendering bubbles + tool cards.
- **`feat(desktop): Phase 1.5` (`5a62c06`)** — IPC permission asker (`aura/desktop/headless.py:IpcAsker`) implementing the `PermissionAsker` Protocol via Future-keyed-by-id; mid-turn `permission_response` drain in the headless main loop; 3-button permission modal in the frontend (No / Yes once / Yes always) with destructive=true red styling and rule_hint preview.
- **`feat(desktop): Phase 2-1`** — frontend migrated from 222-LoC vanilla TS to React 19 + zustand. New layout under `desktop/frontend/src/`: `types.ts` (discriminated `AuraEvent` union covering all 13 wire kinds), `store.ts` (zustand with 11 immutable reducers; streaming-bubble logic preserved), `bridge.ts` (StrictMode-safe `subscribe` returning sync unsubscribe), 6 components (`MessageBubble` / `ToolCard` / `PermissionModal` / `Composer` / `ConversationView` / `App`), `main.tsx` mounts under `<StrictMode>`. Bundle 63.5 KB gzip. IPC contract preserved verbatim — Rust untouched, Python only a docstring drift fix on `aura/desktop/headless.py` (event shapes corrected: `tool_call_*` correlate by `name`, no `id` field on the wire). Foundation for Phase 2-2/3/4/5 (status bar, slash palette, structured tool results, multi-tab).
- **`fix(desktop)` (`01b83bd`)** — one-shot launcher (`make desktop` / `desktop-yes` / `desktop-build`) now installs `--extra all` so `langchain-openai` / `langchain-anthropic` / `langchain-ollama` / `ddgs` are present (previously the headless subprocess crashed at import and the desktop showed "aura exited"); auto-clears port 5173 if a stale `vite` process is holding it; preflight checks rust/cargo-tauri/uv with optional install prompts.
- **`feat(desktop): Phase 2-2`** — status bar via new `aura_state` NDJSON event (Python emits one snapshot at startup + after every `Final`, carrying model + mode + cwd + tokens + pinned + window + last-turn-seconds); React side gains `applyAuraState` slice + `<StatusBar />` segment row + new `<WelcomeScreen />` masthead for empty conversations + `lib/turn-groups.ts` walker grouping tool messages with their assistant turns. **Editorial Terminal redesign**: full `style.css` rewrite (~760 LoC) replacing the Phase 1 vanilla baseline — paper-tone dark surfaces (no pure black), single warm amber accent, IBM Plex Serif (display) + IBM Plex Mono (body) loaded via Google Fonts CDN with `display=swap`, asymmetric magazine grid putting tool calls in a right-margin gutter as marginalia, hairline rules everywhere, restrained motion (3 keyframes total: caret-blink / turn-enter / mount-stagger), inline SVG noise overlay for paper texture. Bundle 64.7 KB gzip (+0.7 KB over Phase 2-1). Design system documented at `docs/superpowers/specs/2026-04-27-aura-desktop-design-system.md`.
...
- **`/team add --backend <kind>` CLI flag** (Track B follow-up): the existing `BackendType` literal (`in_process` | `pane`) is now selectable from the slash command directly — `/team add alice --backend pane` (in-tmux) or `/team add bob` (default in-process). Flag is position-agnostic, validated against the registry before the manager mutates state, and surfaces friendly errors for unknown kinds, missing values, and `pane` outside `$TMUX`. The Python `mgr.add_member(backend_type=...)` API is unchanged — this just unblocks REPL-only users who couldn't drop into a Python shell to pick a backend.
...
- **`/buddy` command + claude-code-ported ASCII sprites** (`f90e18b`): port of claude-code's 9 overlapping species sprites (`duck`, `cat`, `owl`, `turtle`, `rabbit`, `octopus`, `penguin`, `dragon`, `axolotl`) verbatim from `src/buddy/sprites.ts` (5 lines × 12 cols, 3 frames each, `{E}` eye-placeholder substitution). Custom `dog` sprite for Aura's 10th species. 6 hat variants (`crown` / `tophat` / `propeller` / `halo` / `wizard` / `beanie` / `tinyduck`) + 6 eye glyphs (`·` `✦` `×` `◉` `@` `°`) ported as composition tables. Hat compositing skips frames whose line 0 is non-empty (matches claude-code's animation-data-protection contract). `Buddy` dataclass extended with `eye` + `hat` fields; `generate_buddy` rolls them via the same mulberry32 PRNG with the existing `had_recent_error` short-circuit preserved.
- **Continuous IDLE_SEQUENCE animation in `/buddy`** (`26997b8` + `409c0cb`): `/buddy` runs claude-code's exact `IDLE_SEQUENCE = [0, 0, 0, 0, 1, 0, 0, 0, -1, 0, 0, 2, 0, 0, 0]` fidget cycle indefinitely until the user dismisses with Enter. Two concurrent asyncio tasks race: an animation loop that rewrites the 5-line sprite area with ANSI cursor-up escapes (`\033[F`), and a `sys.stdin.readline` waiter offloaded to the executor. First to finish wins; the other is cancelled and drained. 250 ms tick. Static fallback (`/buddy still`) on non-TTY contexts.
- **Buddy `thinking` mood now reachable** (`89f139b`): `pre_model` observer flips mood to `thinking` the moment a user submits a prompt, transitions out automatically when `post_model` lands. Worry-preserving — sticks worried through thinking transition. `default_hooks()` now ships a `pre_model` entry alongside the existing `post_model` + `post_tool` defaults.
- **Animated status-bar buddy glyph** (`1b3b6b6` — V14-SIDEBAR): the buddy glyph in the bottom toolbar now cycles through the IDLE_SEQUENCE hop offsets at the same 250 ms cadence the `/buddy` modal uses, driven by prompt_toolkit's `refresh_interval`. Visible without opening the modal — matches claude-code's "always moving" sidebar feel.
- **`/stats 7d` and `/stats all` journal replay** (`ad0cdbb` — V14-STATS-HISTORY): aggregates `turn_usage` events from the on-disk JSONL journal, grouped by model, with per-model row totals (input / output / cache / turn count) and a footer total. Plain-text aligned table (no rich import in `aura/core/**` per architectural invariant). `/stats` (no arg) keeps the v0.13 current-session view; the explicit window argument unlocks history replay without changing the default behavior.
- **`restrict_tools` whitelist with turn-scoped permission lease** (`a82bdc9` — V14-RESTRICT-TOOLS): skill frontmatter's `restrict-tools: [read_file, grep]` installs a deny-by-default lease for the rest of the turn — every tool not on the whitelist is blocked at the permission hook with `restrict_tools_blocked` audit event. Complements v0.13's `allowed_tools` (permissive auto-allow) — a skill can declare both: "auto-allow these, deny everything else." Internal tools (`ask_user_question`, `enter_plan_mode`, `exit_plan_mode`) are exempt so the loop's own machinery survives. Lease lives on `LoopState.custom["_skill_restrict_lease"]` keyed by install-turn so it self-expires when the turn count advances.
- **FileChanged + CwdChanged hooks with default consumers** (`044d19a` — V14-HOOK-CATALOG): pure-stdlib mtime-polling FileWatcher (1 s default) feeds two new hook protocols with **real** built-in consumers — `make_aura_md_reload_hook` rebuilds `_primary_memory` + `_context` + `_loop` when `~/.aura/AURA.md` changes mid-session, and `make_cwd_rules_reload_hook` re-scans walk-up project layers when `Agent.set_cwd(path)` fires. No half-wired protocol: every Aura agent gets hot-reload by default. Errors degrade to skip-this-tick. `aura_md_reloaded` + `cwd_rules_reloaded` journal events for audit.

### Changed

- **`CommandKind` literal expanded with `"view"`** (`c189b94` + `6be1b36`): display-heavy commands now render their output as a framed modal (rich.Panel + dim border) and block until the user hits Enter. Migrated: `/help`, `/stats`, `/buddy` (animated + static), `/tasks` listing, `/task-get` detail, `/status`, `/diff`, `/log`, `/mcp` status. Action-only commands (`/exit`, `/clear`, `/compact`, `/model`, `/task-stop`, all error / usage messages) keep `kind="print"`. `Ctrl+C` / `Ctrl+D` / pytest's stdin-capture `OSError` all silently dismiss the view.

### Fixed

- **Circular import on `/buddy` registration** (`06c371d`): `aura.core.commands.buddy` now imports `aura.cli.buddy` lazily inside `handle()` rather than at module load. Prevents the package chain `aura.cli.__init__` → `aura.cli.commands` → `aura.core.commands.buddy` → `aura.cli.buddy` from re-entering `aura.cli` before its exports are assigned. Pattern mirrors `aura.core.hooks.must_read_first` and `bash_safety`.
- **Skills loader: inline shell sanitizer + unsupported-frontmatter audit** (`fbc58b8`): inline `` !`cmd` `` directives are now neutralized at render time (replaced with an inert visible marker — the cmd text is preserved as plain prose so the model sees what was meant, but no parser sees the directive prefix at line start). Unrecognized claude-code frontmatter fields emit a `skill_unsupported_frontmatter` journal event with the field names, so silent compat drift is auditable.
- **Permission hook chain: pre_tool decisions merge first-deny-wins** (`6033ab0`): when multiple registered `pre_tool` hooks return decisions in one turn, the first **deny** now wins (was last-wins, which made hook ordering load-bearing in unintended ways). Each per-hook decision emits `pre_tool_hook_decision` carrying hook qualname + tool + allow + reason **before** the merge, so the audit trail shows every hook's contribution even when an earlier hook short-circuited the chain.

### Verification

- **14 deep E2E scenarios** (`27736b0`): pty / subprocess-driven tests pinning the contracts surfaced during V14 work — bypass-mode skips permission layer (semantic mismatch w/ safety-hook), restrict-tools install-turn lifecycle, ResolveOnce dedup, microcompact + auto_compact behavior, AURA.md hot reload, subagent journal completeness, 50-turn long-session stability, skill inline-shell neutralization, random-seed resilience. All `@pytest.mark.e2e`-gated. 14/14 pass under default order + seed=42 + seed=99999 + xdist -n 4.

### Migration notes (SDK consumers)

- **`CommandKind` Literal extended**: callers pattern-matching on `result.kind` should add a `"view"` arm (or a pass-through to whatever default branch handles `"print"`).
- **`Buddy` dataclass gained `eye` + `hat` fields**: default to `""` for backward compat with cached v0.13.0 state.
- **`AURA_BUDDY_TICK_MS` env var**: tunes `/buddy` modal + status-bar buddy animation tick rate (default 250 ms).
- **`Agent.__init__` accepts `loop_state_provider` for `SkillTool`** (non-breaking): the v0.14 `restrict_tools` lease needs the live `LoopState` to stamp install-turn. Defaults to a no-op closure when not wired (unit-test path).
- **New journal events**: `restrict_tools_blocked`, `restrict_tools_lease_installed`, `aura_md_reloaded`, `cwd_rules_reloaded`, `file_changed`, `cwd_changed`, `pre_tool_hook_decision`, `skill_unsupported_frontmatter`. Existing scrapers should extend, not rewrite.
- **`pyproject.toml` version**: bumped from `0.11.0` (which had stayed pinned across v0.12.0 / v0.13.0 tags despite the changelog rolling forward) to `0.14.0`. Now aligned with the git tag.
