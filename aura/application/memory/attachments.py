...
:mod:`aura.application.memory` ``__init__`` docstring) is that
:meth:`aura.application.memory.context.Context.build` is the single site where
...
def build_mcp_resource_message(
    *,
    ...
    used by :meth:`aura.application.memory.context.Context.build`: an XML-ish
