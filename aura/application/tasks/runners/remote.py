...
runner spawns a fresh Python subprocess invoking ``python -m cli
teammate`` (or any equivalent module-level entrypoint) and tracks its
exit code. Communication with the parent is exclusively via the on-disk
JSONL mailbox under the shared storage root — the subprocess never
holds a back-channel IPC connection to the parent, mirroring the
constraints of the pane backend that originally motivated the design.
"""
...
DEFAULT_ENTRYPOINT_MODULE = "cli"
DEFAULT_ENTRYPOINT_SUBCOMMAND = "teammate"
...
class RemoteAgentTask:
    ...
    def _argv(self) -> list[str]:
        ...
        Mirrors the ``teammate`` subcommand in :mod:`cli.__main__`: every
