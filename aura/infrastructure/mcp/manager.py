...
2. **Aura metadata attachment.** Discovered tools get
   :func:`aura.infrastructure.mcp.adapter.add_aura_metadata` applied so the permission
...
3. **Prompt → Command bridging.** MCP prompts are wrapped as Aura commands
   via :func:`aura.infrastructure.mcp.adapter.make_mcp_command` so ``/help`` and
...
4. **Resource catalogue + on-demand read.** MCP servers expose URI-identified
   *resources* (files, DB snapshots, DOC pages ...). The manager lists them
   ...
   CLI ``@server:uri`` attachment preprocessor can (a) discover the
   catalogue and (b) route ``read_resource`` calls to the right server's
   session. Resources are NOT eagerly fetched — a library-side call to
   ``session.read_resource(uri)`` runs only when the preprocessor expands
   a ``@`` mention.
...
class MCPManager:
    def __init__(
        ...
        *,
        ...
        # against str URIs anyway). Consumed by :meth:`resources_catalogue`
        # which the CLI ``@server:uri`` attachment preprocessor queries.
    ...
    async def read_resource(self, uri: str) -> dict[str, Any]:
        ...
        :func:`aura.infrastructure.mcp.adapter.normalize_resource_contents` so the
        ...
        the CLI ``@server:uri`` preprocessor surfaces this message back
        to the operator so they can correct the mention.
