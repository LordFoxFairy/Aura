"""task_stop — cancel a still-running subagent.
...
The actual "mark cancelled on the record" happens inside ``run_task``
when it catches the ``CancelledError`` — that guarantees the record
always reflects the true lifecycle regardless of who cancelled it
(this tool, ``Agent.close()``, or Ctrl+C). This tool only arms the
cancel; it doesn't touch the store directly.
"""
...
class TaskStop(BaseTool):
    ...
    store: TasksStore
    # See task_create for why ``running`` is a PrivateAttr rather than a
    # pydantic field — identity-sharing with the owning Agent.
