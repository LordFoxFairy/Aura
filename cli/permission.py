...
modules — this file routes to them:
  ...
  * :mod:`cli.permission_bash`    — ``bash`` / ``bash_background``
    (syntax-highlighted command + dangerous-arg banner)
  * :mod:`cli.permission_write`   — ``write_file`` / ``edit_file``
    (diff preview for edits, size + head preview for writes)
  * :mod:`cli.permission_generic` — everything else (the original
...
from cli.permission_bash import run_bash_permission
from cli.permission_generic import (
    _TOOL_VERB,
    _build_explanation,
    _run_widget,
    _tool_title,
...
from cli.permission_write import run_write_permission
