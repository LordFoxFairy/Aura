...
async def test_hookchain_empty_is_noop() -> None:
    ...
    await chain.run_pre_model(history=history)
    await chain.run_post_model(ai_message=ai_msg, history=history)
    decision = await chain.run_pre_tool(tool=_stub_tool, params=params)
    final = await chain.run_post_tool(tool=_stub_tool, params=params, result=result)
...
async def test_pre_model_sees_history_and_can_mutate() -> None:
    async def inject(*, history: list[BaseMessage]) -> None:
...
async def test_post_model_sees_ai_message() -> None:
    ...
    await chain.run_post_model(ai_message=ai_msg, history=[])
...
async def test_pre_tool_short_circuits_with_tool_result() -> None:
    ...
    result = await chain.run_pre_tool(tool=_stub_tool, params=_P())
...
async def test_pre_tool_first_short_circuit_wins() -> None:
    ...
    async def first(*, tool: AuraTool, params: BaseModel) -> ToolResult | None:
        call_log.append("first")
        return decision

    async def second(*, tool: AuraTool, params: BaseModel) -> ToolResult | None:
...
async def test_post_tool_chains_in_order() -> None:
    async def append_a(*, tool: AuraTool, params: BaseModel, result: ToolResult) -> ToolResult:
        out = list(result.output) if isinstance(result.output, list) else []
        out.append("a")
        return ToolResult(ok=True, output=out)

    async def append_b(*, tool: AuraTool, params: BaseModel, result: ToolResult) -> ToolResult:
        out = list(result.output) if isinstance(result.output, list) else []
        out.append("b")
        return ToolResult(ok=True, output=out)
    ...
    final = await chain.run_post_tool(
        tool=_stub_tool, params=_P(), result=ToolResult(ok=True, output=[])
    )
...
async def test_multiple_hooks_of_same_type_run_in_registration_order() -> None:
    ...
    async def first(*, history: list[BaseMessage]) -> None:
        call_log.append("first")

    async def second(*, history: list[BaseMessage]) -> None:
        call_log.append("second")

    chain = HookChain(pre_model=[first, second])
    await chain.run_pre_model(history=[])
...
async def test_pre_tool_chain_returns_none_when_all_return_none() -> None:
    async def pass_through(*, tool: AuraTool, params: BaseModel) -> ToolResult | None:
        return None
